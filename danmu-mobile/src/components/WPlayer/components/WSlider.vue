    ...
    setup(props, ctx) {
        ...
        const slideTimeLine = () => {
            blockRef.value!.ontouchstart = function () {
                document.ontouchmove = function (e) {
                    const activeHeight = (e.changedTouches[0].clientX - sliderRef.value!.getBoundingClientRect().left);
                    let percentage = Math.round((activeHeight / sliderRef.value!.clientWidth) * 100) / 100;
        ...
        const slideTimeLineVertical = () => {
            blockRef.value!.ontouchstart = function () {
                document.ontouchmove = function (e) {
                    //计算新的百分比
                    const activeHeight = sliderRef.value!.clientHeight - (e.changedTouches[0].clientY - sliderRef.value!.getBoundingClientRect().top);
                    let percentage = Math.round((activeHeight / sliderRef.value!.clientHeight) * 100) / 100;
                    percentage = Math.max(0, percentage);
                    percentage = Math.min(percentage, 1);
